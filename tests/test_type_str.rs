...
fn test_type_str() {
    ...
    assert_eq!(<&str>::decode(b"jankincai\r\n", None, Some(&fattr)).is_err(), true);
}


#[test]
fn test_type_str_byte_count() {
    // example - #[jppe(byte_count=1)]  default
    let (input, value) = <&str>::decode(b"\x09jankincai", None, None).unwrap();
    assert_eq!(value, "jankincai");
    assert_eq!(input.is_empty(), true);
    let mut buf = Vec::new();
    ...
    let (input, value) = <&str>::decode(b"\x09jankincai", None, Some(&fattr)).unwrap();
    assert_eq!(value, "jankincai");
    assert_eq!(input.is_empty(), true);
    let mut buf = Vec::new();
    value.encode(&mut buf, None, Some(&fattr));
    ...
    let (input, value) = <&str>::decode(b"\x00\x09jankincai", None, Some(&fattr)).unwrap();
    assert_eq!(value, "jankincai");
    assert_eq!(input.is_empty(), true);
    let mut buf = Vec::new();
    value.encode(&mut buf, None, Some(&fattr));
    ...
    let (input, value) = <&str>::decode(b"\x00\x00\x00\x09jankincai", None, Some(&fattr)).unwrap();
    assert_eq!(value, "jankincai");
    assert_eq!(input.is_empty(), true);
    let mut buf = Vec::new();
    value.encode(&mut buf, None, Some(&fattr));
    ...
    let (input, value) = <&str>::decode(b"\x00\x00\x00\x00\x00\x00\x00\x09jankincai", None, Some(&fattr)).unwrap();
    assert_eq!(value, "jankincai");
    assert_eq!(input.is_empty(), true);
    let mut buf = Vec::new();
    value.encode(&mut buf, None, Some(&fattr));
