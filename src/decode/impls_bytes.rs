    ...
    if let Some(fattr) = fattr {
        if fattr.linend {
            ...
            for linend_value in linend_value_list {
                if let Ok((input, value)) = parse_subsequence(input, linend_value)
                {
                    value_tmp = Some(value);
                    input_tmp = input;
                    break;
...
mod tests {
    ...
    fn test_decode_bytes() {
        ...
        let (input, value) = <&[u8]>::decode(b"12\r", None, Some(&fattr)).unwrap();
        // println!("{:?} {:?}", value, input);
        assert_eq!(value, b"12\r");
        assert_eq!(input.is_empty(), true);
